...
<form method="post" action=" {{asset ('users/$id')}} ">
    <!-- <form method="post" action=" {{route ('users.show', $id)}} "> -->
    @csrf
    @method("put")
    <div class=" form-group row">
        <label for="amen" class="col-sm-2 col-form-label">Name</label>
        <div class="col-sm-10">
            <input type="text" value="name{{$id}}" class="form-control" id="n" name="name" placeholder="name">
        </div>
    </div>
    <div class="form-group row">
        <label for="email" class="col-sm-2 col-form-label">Email</label>
        <div class="col-sm-10">
            <input type="email" value="email{{$id}}" class="form-control" id="email" name="email" placeholder="email">
        </div>
    </div>
    <div class="form-group row">
        <label for="phone" class="col-sm-2 col-form-label">Phone</label>
        <div class="col-sm-10">
            <input type="integer" value="phone{{$id}}" class="form-control" id="phone" name="phone" placeholder="phone">
        </div>
    </div>
